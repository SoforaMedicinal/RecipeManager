...
{
    public partial class RecipeDiary : Form
    {
        ...
        {
            DialogResult res = MessageBox.Show("Сохранить файл?", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            switch (res)
            {
                case DialogResult.Yes:
                    var data = new RecipeManagerData
                    {
                        ...
                        DishCategory = _storage.GetGroups().ToList()
                    };
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!File.Exists($@"{filePath}"))
            {
                DialogResult res = MessageBox.Show($"Файл '{filePath}' не найден. Возможно адрес расположения файла был изменен?", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void mainListView_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            ...
            lvIngredientsRecipe.Items.Clear();
            for (int i = 0; i < _storage.GetRecipe()[index].IngredientStorage.Count(); i++)
            {
                lvIngredientsRecipe.Items.Add(new ListViewItem(new string[] {
                    _storage.GetRecipe()[index].IngredientStorage[i].Name}));
            }
        ...
        private void RecipeContainer_Changed(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _storage.GetRecipe().Count(); i++)
            {
                mainListView.Items.Add(new ListViewItem(new string[] {
                    ...
                    _storage.GetRecipe()[i].DishCategory.Name}));
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (mainListView.SelectedItems.Count == 0)
            {
                MessageBox.Show("Пожалуйста, выберите один пункт из списка", "Ошибка удаления",
        ...
        private void btnDeleteIngr_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    _storage.GetIngredient().Delete(index);
                }
                else
                {
                    MessageBox.Show("Этот ингредиент уже есть в рецепте", "Ошибка удаления",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
        ...
        {
            var createResult = RecipeManager.DishCategory.Create(txtNameGrou.Text);
            if (!createResult.Succeeded)
            {
        ...
        private void btnDeleteGrou_Click(object sender, EventArgs e)
        {
            if (lvGroup.SelectedItems.Count == 0)
            {
                MessageBox.Show("Пожалуйста, выберите ингредиент из списка", "Ошибка удаления",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else
            {
                ...
                {
                    _storage.GetGroups().Delete(index);
                }
                else
                {
                    MessageBox.Show("Рецепт уже есть в этой группе", "Ошибка удаления",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }
        ...
        private void btnEditGrou_Click(object sender, EventArgs e)
        {
            if (lvGroup.SelectedItems.Count == 0)
            {
                MessageBox.Show("Пожалуйста, выберите группу", "Ошибка удаления",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else
        ...
        private bool CheckingRecipeForAnGroup(DishCategory dishCategory)
        {
            return _storage.GetRecipe().Any(r => r.DishCategory.Name == dishCategory.Name);
        }
        private bool CheckingRecipeForAnIngredient(Ingredient ingredient)
        {
            foreach (var recipe in _storage.GetRecipe())
            {
                if (recipe.IngredientStorage.Any(i => i.Name == ingredient.Name)) return true;
            }
            return false;
        }

        private void LvIngredientsRecipe_SelectedIndexChanged(object sender, EventArgs e)
        {
